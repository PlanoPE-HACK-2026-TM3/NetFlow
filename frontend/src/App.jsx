...
function App() {
  ...
  const [chatInput, setChatInput] = useState(
    "What is my current portfolio outlook?",
  );
  ...
  const handleSearch = async (event) => {
    ...
    setError(null);
    try {
      const { data } = await api.post("/properties/search", {
        location,
        limit: 50,
        allow_demo: false,
      });
      ...
      showError(
        detail ||
          "No listings found for that location. Try a supported ZIP or city.",
      );
    } finally {
      setLoading(false);
    }
  ...
  const handleAnalyze = async (property) => {
    ...
    try {
      ...
      showError(
        "Analysis failed. The model may be loading — try again shortly.",
      );
    } finally {
      setAnalyzingId(null);
    }
  ...
  const handleAddPortfolio = async (property) => {
    if (portfolio.some((p) => p.property_id === property.property_id)) {
      showError("Property is already in your portfolio.");
      return;
    }
    try {
      ...
      await loadPortfolio();
      showSuccess("Property added to portfolio successfully!");
    } catch {
    ...
    <div className="app-shell">
      ...
      >
        <AnimatePresence>
          ...
          {successMessage && (
            <motion.div
              className="error-banner success-banner"
              initial={{ opacity: 0, x: 50 }}
              animate={{ opacity: 1, x: 0 }}
              exit={{ opacity: 0, x: 50 }}
              style={{
      ...
      <header className="topbar">
        <div>
          ...
          <p className="subtitle">
            Search opportunities, score risk, and track predictive insights.
          </p>
        </div>
        <button
        ...
        <article className="metric-card">
          ...
          <p>
            {portfolioTotalValue > 0
              ? `$${(portfolioTotalValue / 1e6).toFixed(2)}M`
              : "$0"}
          </p>
          <span>
            {portfolio.length} tracked{" "}
            {portfolio.length === 1 ? "property" : "properties"}
          </span>
        </article>
      </section>
        ...
        <div className="properties-panel">
          ...
          {properties.length === 0 && !loading && (
            <p className="muted empty-hint">
              Search a location above to discover properties.
            </p>
          )}
            ...
            return (
              ...
              >
                ...
                <div className="property-source-row">
                  <span
                    className={`source-pill ${property.source === "demo" ? "demo" : "real"}`}
                  >
                ...
                <div className="card-actions">
                  ...
                  <button
                    onClick={() => handleAddPortfolio(property)}
                ...
                {analysis && (
                  <div className="analysis-box">
                    <div className="score-row">
                      <span className="score-badge invest">
                        Invest {analysis.investment_score}/100
                      </span>
                      <span className="score-badge risk">
                        Risk {analysis.risk_score}/100
                      </span>
                      <span
                        className={`rec-badge rec-${analysis.recommendation}`}
                      >
                    ...
                    <p className="analysis-metric">
                      1Y Outlook:{" "}
                      <strong>
                        {analysis.projected_12m_change_percent > 0 ? "+" : ""}
                        {analysis.projected_12m_change_percent}%
                      </strong>{" "}
            ...
            <div className="chat-body">
              {chatHistory.length === 0 ? (
                <div className="chat-empty">
                  ...
                  <p>
                    Ask NETFLOW anything about your portfolio, market trends, or
                    investment risk.
                  </p>
      ...
      <AnimatePresence>
        {showPortfolioModal && (
          <motion.div
            ...
            exit={{ opacity: 0 }}
            onClick={(e) =>
              e.target === e.currentTarget && setShowPortfolioModal(false)
            }
          >
            <motion.div
              className="modal"
            ...
            >
              ...
              <div className="portfolio-summary">
                <span>
                  {portfolio.length}{" "}
                  {portfolio.length === 1 ? "property" : "properties"} tracked
                </span>
                <span className="portfolio-total">
                  Total: ${portfolioTotalValue.toLocaleString()}
                </span>
              </div>
              {portfolio.length === 0 && (
