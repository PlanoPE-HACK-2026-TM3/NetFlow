...
async function persistLog(entry: LogEntry) {
  ...
  return new Promise<void>((res) => {
    ...
    // file opens the DB first, also create every store db.ts expects —
    // otherwise db.ts opens a pre-existing v2 DB, skips onupgradeneeded,
    // and throws NotFoundError on its first transaction.
