      ...
      <div style={{padding:"13px 13px 10px"}}>
        ...
        <div style={{display:"flex",alignItems:"flex-start",gap:"7px",marginBottom:"8px"}}>
          ...
          <div style={{flex:1,minWidth:0}}>
            <div style={{fontSize:"12px",fontWeight:700,color:"var(--t1)",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{p.address}</div>
          ...
          <div style={{flex:1,display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:"6px"}}>
            ...
            <Tip right={isRight} text="Cap Rate = NOI ÷ Price × 100. NOI = Rent × 12 × 0.65 (35% expenses deducted). Measures unleveraged return. Target ≥ 6% for strong investment.">
              <div style={{display:"flex",flexDirection:"column",gap:"2px"}}>
                ...
                <div style={{fontSize:"14px",fontWeight:800,fontFamily:"'JetBrains Mono',monospace",color:crC}}>{p.cap_rate}%</div>
              </div>
            </Tip>
            ...
            <Tip text="Cash Flow = Rent − PITI − Operating Expenses (35% of rent). PITI = Principal, Interest, Tax, Insurance. Positive = property covers all costs and profits monthly.">
              <div style={{display:"flex",flexDirection:"column",gap:"2px"}}>
                ...
                <div style={{fontSize:"14px",fontWeight:800,fontFamily:"'JetBrains Mono',monospace",color:cfC}}>${p.cash_flow}/mo</div>
              </div>
            </Tip>
            ...
            <Tip right text="GRM = Price ÷ Annual Rent. Lower = better value. Under 100× excellent, 100–130× good, 130–160× fair, above 160× overpriced. Quick market comparison tool.">
              <div style={{display:"flex",flexDirection:"column",gap:"2px"}}>
                ...
                <div style={{fontSize:"14px",fontWeight:800,fontFamily:"'JetBrains Mono',monospace",color:"var(--pri-hi)"}}>{p.grm}×</div>
              </div>
            </Tip>
          </div>
        ...
        <div style={{marginTop:"6px",fontSize:"9px",fontWeight:600,color:selected?"var(--pri-hi)":"var(--t3)"}}>
          💬 {selected?"AI Property Analyst open →":"Tap card to open AI Analyst"}
        </div>
      </div>
    </div>
  );
...
// ── AI Score Ring Chart — all 10 properties as gauge rings ──────
export function ScoreStrip({ properties }:{ properties:Property[] }) {
  // Ring dimensions
  const R = 34, STROKE = 6, SIZE = (R+STROKE)*2+4;
  const CIRC = 2 * Math.PI * R;

  return (
    <div className="score-strip fade-in">

      {/* Header */}
      <div style={{display:"flex",alignItems:"center",gap:"8px",marginBottom:"16px"}}>
        <span style={{fontSize:"18px",lineHeight:1}}>🏆</span>
        <div>
          <div style={{fontSize:"13px",fontWeight:700,color:"var(--t2)"}}>AI Score — Top {properties.length}</div>
          <div style={{fontSize:"11px",color:"var(--t3)",fontWeight:400}}>NetFlow investment score out of 100</div>
        </div>
      </div>

      {/* Rings row — horizontally scrollable on small screens */}
      <div style={{display:"flex",gap:"clamp(8px,2vw,20px)",overflowX:"auto",paddingBottom:"4px",alignItems:"flex-start",
        /* hide scrollbar */ scrollbarWidth:"none"}}>
        {properties.map((p, i) => {
          const { icon, color } = scoreInfo(p.ai_score);
          const ci   = (p.rank-1) % 5;
          const ringC = NOTE_TOPS[ci];   // note colour for the ring stroke
          const fill  = (p.ai_score / 100) * CIRC;
          // short address label: first 2 words
          const label = p.address.split(" ").slice(0,2).join(" ");
          const rankLabel = `#${p.rank} ${label}`;

          return (
            <div key={p.address} style={{display:"flex",flexDirection:"column",alignItems:"center",gap:"7px",
              flexShrink:0, minWidth:"68px", flex:"1 1 0"}}>
              ...
              <div style={{position:"relative",width:`${SIZE}px`,height:`${SIZE}px`}}>
                <svg width={SIZE} height={SIZE} viewBox={`0 0 ${SIZE} ${SIZE}`}
                  ...
                  <circle
                    ...
                    style={{transition:`stroke-dasharray .9s ${i*0.07}s ease`}}
                  />
                </svg>
                {/* Centre content: icon + score */}
                <div style={{position:"absolute",inset:0,display:"flex",flexDirection:"column",
                  alignItems:"center",justifyContent:"center",gap:"1px"}}>
                  <span style={{fontSize:"16px",lineHeight:1}}>{icon}</span>
                  <span style={{fontSize:"14px",fontWeight:800,color,fontFamily:"'JetBrains Mono',monospace",lineHeight:1}}>
                    {p.ai_score}
                  </span>
                </div>
              </div>

              {/* Label below ring: "#1 8361 Mockingbird" split into 2 lines */}
              <div style={{textAlign:"center",lineHeight:1.35}}>
                <div style={{fontSize:"10px",fontWeight:700,color:"var(--t3)",fontFamily:"'JetBrains Mono',monospace"}}>
                  #{p.rank} {p.address.split(" ")[0]}
                </div>
                <div style={{fontSize:"10px",fontWeight:500,color:"var(--t3)"}}>
  ...
  return (
    <div className="rank-note">
      ...
      <button onClick={()=>{ setOpen(v=>!v); log.ui("Rank formula note toggled",{open:!open}); }}
        ...
        <span style={{fontSize:"12px",fontWeight:700}}>🧮 How is Rank Calculated?</span>
      ...
      {open && (
        <div style={{padding:"13px 14px"}} onClick={e=>e.stopPropagation()}>
          <p style={{fontSize:"11px",color:"var(--t2)",marginBottom:"10px",lineHeight:1.65}}>
            Each property receives an AI score 0–100 from <strong style={{color:"var(--pri-hi)"}}>local Ollama (llama3)</strong> using a weighted rubric. Ranked #1–10 by score. Falls back to rule-based scoring if Ollama is offline.
          </p>
          {dims.map(r=>(
          ...
          <div style={{marginTop:"10px",padding:"9px 11px",borderRadius:"8px",background:"var(--bg-raise)",border:"1px solid var(--bd)",fontSize:"10px",color:"var(--t3)",lineHeight:1.65}}>
            ...
            Scored by <strong style={{color:"var(--pri-hi)"}}>Ollama llama3</strong> · rule-based fallback when offline
          </div>
        </div>
      )}
    </div>
