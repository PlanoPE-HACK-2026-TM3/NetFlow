...
const scoreInfo = (s: number) =>
  s >= 85
    ? { c: "#10b981", label: "Excellent" }
    : s >= 70
      ? { c: "#3b82f6", label: "Strong" }
      : s >= 55
        ? { c: "#f59e0b", label: "Good" }
        : { c: "#f43f5e", label: "Fair" };

const cfColor = (v: number) =>
  v > 300 ? "#10b981" : v > 0 ? "#f59e0b" : "#f43f5e";
const capColor = (v: number) =>
  v >= 6 ? "#10b981" : v >= 4.5 ? "#f59e0b" : "#3b82f6";
...
function Tip({
  ...
  return (
    ...
    >
      {children}
      <div className="tip-box" role="tooltip">
        {text}
      </div>
    </div>
  );
...
const PropertyCard = memo(function PropertyCard({
  ...
  const metrics = [
    {
      ...
      label: "Cap Rate",
      c: crC,
      tip: "Cap Rate = NOI ÷ Price × 100. Target ≥ 6% for strong investment.",
    },
    {
      key: "cf",
      val: `$${p.cash_flow}`,
      label: "Cash Flow",
      c: cfC,
      tip: "Cash Flow = Rent − PITI − 35% opex. Monthly profit after all costs.",
    },
    {
      key: "grm",
      val: `${p.grm}×`,
      label: "GRM",
      c: "var(--pri-hi)",
      tip: "GRM = Price ÷ Annual Rent. Under 100× excellent, 130× fair.",
    },
  ];
  ...
  return (
    <article
      ...
      onClick={() => {
        onSelect(p);
        log.ui("Card selected", { rank: p.rank });
      }}
    >
      {/* ══ PHOTO ═══════════════════════════════════════════ */}
      <div
      ...
      >
        ...
        >
          ...
          >
            <path
              d="M45 5L5 38h9v34h20V52h22v20h20V38h9L45 5z"
              fill={`${accent}35`}
              stroke={`${accent}70`}
              strokeWidth="1.5"
              strokeLinejoin="round"
        ...
        <div style={{ position: "absolute", top: 12, right: 12, zIndex: 10 }}>
          <Tip right text="AI generated score based on property metrics">
            <div
              style={{
            ...
            >
              <div style={{ position: "relative", width: 24, height: 24 }}>
                <svg width="24" height="24" viewBox="0 0 28 28" style={{ transform: "rotate(-90deg)" }}>
                  <circle cx="14" cy="14" r="12" fill="none" stroke="rgba(255,255,255,0.2)" strokeWidth="2.5" />
                  <circle cx="14" cy="14" r="12" fill="none" stroke={scoreC} strokeWidth="2.5"
                    strokeDasharray={`${(p.ai_score / 100) * (2 * Math.PI * 12)} ${2 * Math.PI * 12}`} strokeLinecap="round"
                    style={{ transition: "stroke-dasharray .9s ease" }} />
                </svg>
                <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 8 }}>
                  {p.ai_score >= 85 ? "🏆" : p.ai_score >= 70 ? "⭐" : p.ai_score >= 55 ? "✅" : "⚠️"}
                </div>
              </div>
              <div style={{ display: "flex", flexDirection: "column", gap: 1 }}>
                ...
                >
                  {scoreLabel} <span style={{ fontSize: 8, color: "#fff" }}>ℹ️</span>
                </span>
              </div>
            </div>
          </Tip>
      ...
      <div style={{ padding: "18px 18px 14px" }}>
        ...
        >
          {[
            ...
            {
              icon: "📐",
              label: p.sqft ? `${(p.sqft / 1000).toFixed(1)}k sqft` : "—",
            },
          ].map((s) => (
            <span
              key={s.label}
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            <div style={{ position: "relative", width: 36, height: 36 }}>
              <svg width="36" height="36" viewBox="0 0 36 36" style={{ transform: "rotate(-90deg)" }}>
                ...
                <circle cx="18" cy="18" r="15" fill="none" stroke={scoreC} strokeWidth="3"
                  strokeDasharray={`${(p.ai_score / 100) * (2 * Math.PI * 15)} ${2 * Math.PI * 15}`} strokeLinecap="round"
                  style={{ transition: "stroke-dasharray .9s ease" }} />
              </svg>
            ...
            <div>
              <div style={{ fontSize: 11, fontWeight: 700, color: "var(--t1)", display: "flex", alignItems: "center", gap: 4 }}>
                NetFlow Score
                <Tip text="AI generated score based on property metrics">
                  <span style={{ cursor: "help", fontSize: 11, color: "var(--t3)" }}>ℹ️</span>
                </Tip>
              </div>
              <div style={{ fontSize: 10, color: "var(--t3)" }}>{scoreLabel} fit</div>
            </div>
          </div>
          <div style={{ fontSize: 18, fontWeight: 800, color: scoreC, fontFamily: "'JetBrains Mono',monospace" }}>
            {p.ai_score}
          ...
          >
            ...
            >
              {[
                ...
                [
                  "Lot Size",
                  p.lot_size ? `${p.lot_size.toLocaleString()} sqft` : "N/A",
                ],
              ].map(([l, v]) => (
                <div key={l}>
                  <div
            ...
            <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
              {[
                [
                  "Zillow",
                  `https://www.zillow.com/homes/${encodeURIComponent(p.address + " " + p.zip_code)}`,
                ],
                ["Redfin", `https://www.redfin.com/zipcode/${p.zip_code}`],
                [
                  "Realtor",
                  `https://www.realtor.com/realestateandhomes-search/${p.zip_code}`,
                ],
              ].map(([l, u]) => (
                <a
                  key={l}
...
/* ── Grid export ─────────────────────────────────────────────── */
export default memo(function PropertyGrid({
  properties,
  onSelectProperty,
  selectedProperty,
  favorites,
