...
export default function Home() {
  ...
  const loadHistories = useCallback(async(username:string)=>{
    const [lh,sh] = await Promise.all([getLoginHistory(username),getSearchHistory(username)]);
  ...
  const handleSearch = async(params:SearchParams & {prompt_text?:string})=>{
    ...
    try {
      ...
      if(!res.body) return;
      const reader=res.body.getReader(); const dec=new TextDecoder(); let partial:Partial<SearchResult>={};
      ...
      <div className="app-body">
        ...
        <aside className={`sidebar${sidebarOpen?" open":""}`}>
          ...
          <SearchPanel onSearch={handleSearch} loading={loading} statusMsg={statusMsg} searchHistory={searchHist}
            onHistorySelect={prompt=>handleSearch({zip_code:"75070",budget:450000,property_type:"SFH",min_beds:3,strategy:"LTR",prompt_text:prompt})}/>
        </aside>
